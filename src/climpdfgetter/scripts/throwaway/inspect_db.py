...
def get_first_row(db_name, user, password, host="localhost", port="5432", table_name="semantic_scholar"):
    try:
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python inspect_db.py <dbname> <user> <password> [host] [port] [table_name]")
        sys.exit(1)
    ...
    get_first_row(dbname, user, password, host, port, table_name)
